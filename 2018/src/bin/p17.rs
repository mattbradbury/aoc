...
fn flow(mut water_map: WaterMap) -> usize {
    ...
    while let Some(fall) = falls.last() {
        ...
        let right_wall = loop {
            ...
            if right == Clay { break Some((x + 1, y)); };
        };
...
fn is_abyss(water_map: &mut WaterMap, mode:Mode, start: (usize, usize), max_y: usize) -> ReturnType {
    ...
    match mode {
        LookSide(look_right) => {
            loop {
                ...
                if down == Flowing { return is_abyss(water_map, LookDown, (next_x, y + 1), max_y); };
                let side = *water_map.entry((next_x,y)).or_default();
                if side == Clay { return Wall}
                x = next_x;    
            }
        ...
        LookDown => {
            loop {
                ...
                if down == Clay || down == Still {      
                    ...
                    if left == Abyss && right == Abyss{ return Abyss; }
                };
                if y > max_y { return Abyss };
                y += 1;
            }
        },
    };
