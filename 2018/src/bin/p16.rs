    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        ...
        Ok(Sample {
            before: lines
                ...
                .as_slice()
                .try_into()?,
            instr: lines.next().unwrap().try_into()?,
            after: lines
                .next()
                .unwrap()
                .split('[')
...
fn build_func_vec() -> Vec<(String, Operation)> {
    ...
    let addr: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = i[args[0]] + i[args[1]];
        i
    };
    let addi: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = i[args[0]] + args[1];
        i
    };
    let mulr: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = i[args[0]] * i[args[1]];
        i
    };
    let muli: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = i[args[0]] * args[1];
        i
    };
    let banr: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = i[args[0]] & i[args[1]];
        i
    };
    let bani: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = i[args[0]] & args[1];
        i
    };
    let borr: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = i[args[0]] | i[args[1]];
        i
    };
    let bori: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = i[args[0]] | args[1];
        i
    };
    let setr: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = i[args[0]];
        i
    };
    let seti: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = args[0];
        i
    };
    let gtir: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = if args[0] > i[args[1]] { 1 } else { 0 };
        i
    };
    let gtri: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = if i[args[0]] > args[1] { 1 } else { 0 };
        i
    };
    let gtrr: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = if i[args[0]] > i[args[1]] { 1 } else { 0 };
        i
    };
    let eqir: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = if args[0] == i[args[1]] { 1 } else { 0 };
        i
    };
    let eqri: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = if i[args[0]] == args[1] { 1 } else { 0 };
        i
    };
    let eqrr: Operation = |mut i: [usize; 4], args: [usize; 3]| {
        i[args[2]] = if i[args[0]] == i[args[1]] { 1 } else { 0 };
        i
    };
...
fn oper_match(mut opers: Vec<(String, Operation)>, sample: &Sample) -> Vec<(String, Operation)> {
    opers.retain(|(_, o)| o(sample.before, sample.instr.args) == sample.after);
    opers
}
...
fn part2((mut samples, instrs): (Vec<Sample>, Vec<Instruction>)) {
    ...
    let regs = instrs
        .iter()
        .fold([0, 0, 0, 0], |r, i| mapping[&i.opcode](r, i.args));
    println!("Part2: {:?}", regs)
}
